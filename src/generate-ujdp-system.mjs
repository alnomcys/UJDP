...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'ujdp', 'Ujdp', and 'UJDP'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.ujdp/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.ujdp/g),
   ...
   * Rename files.
   *
   * Rename files that had ujdp in their name, such as
   * css/ujdp.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*ujdp*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('ujdp', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-ujdp-system.mjs`);
